...
# DON'T EDIT
COMMAND_TEMPLATE="python2 run.py -u %s -n '%s' -a %s -f %s -fc %s -eg %s"
...
# - FAST IV FLAG is a switch to use the new fast iv scanning (true/false)
# - MAX FAILED COUNT is the max number of failed scans before terminating the test (default is "5")
# - MAX EMPTY GMO is the max number of empty GMO scans before terminating the test (default is "5")
declare -A DEVICE_COMMANDS
DEVICE_COMMANDS=(
	["DEVICE NAME 1"]=$(printf "${COMMAND_TEMPLATE}" "UUID 1" "DEVICE NAME 1" "ACCOUNT MANAGER FLAG 1" "FAST IV FLAG 1" "MAX FAILED COUNT 1" "MAX EMPTY GMO 1")
	["DEVICE NAME 2"]=$(printf "${COMMAND_TEMPLATE}" "UUID 2" "DEVICE NAME 2" "ACCOUNT MANAGER FLAG 2" "FAST IV FLAG 2" "MAX FAILED COUNT 2" "MAX EMPTY GMO 2")
)
...
do
	...
	do
		if (tmux ls | grep -wq "$i") >/dev/null 2>/dev/null; then
